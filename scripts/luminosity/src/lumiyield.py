...
def analysis():
    ...
    if (not COIN_PS == None):
        ...
        c_Raw_SysIn = c.add_cut(tree["CTime_CoinTime_RAW_ROC2"], "c_Raw_SysIn")
        c_Raw_SysOut = c.add_cut(tree["CTime_CoinTime_RAW_ROC2"], "c_Raw_SysOut")
    ...
    if (not COIN_PS == None):
        track_info = {
            ...
            "CoinBlocking_SysIn" : (len(c_Raw_SysIn)),
            "CoinBlocking_SysOut" : (len(c_Raw_SysOut)),
        }
...
def main():
    ...
    if (((not (COIN_PS == None) )or(not (HMS_PS == None) )) and (not (SHMS_PS == None)) ): 
    ...
    #else if (SHMS_PS == None):
    #    data["HMS3of4ELT"] = scalers["HMS3of4ELT"]
    #    data["HMS3of4ELT_err"] = scalers["HMS3of4ELT_err"]
